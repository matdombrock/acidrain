    ...
    unsafe fn player_collide(&mut self, cache: Pos) -> bool {
        ...
        for dy in 0..PLAYER_SIZE {
            for dx in 0..PLAYER_SIZE {
                let wx = (self.pos.x + dx as i32) as usize;
                let wy = (self.pos.y + dy as i32) as usize;
...
unsafe fn start() {
    *PALETTE = [0x002200, 0x557755, 0xAAFFAA, 0xCCFFCC];
    ...
    for _ in 0..GOLD_COUNT {
        let x = ST.rng.i32(0..(WORLD_SIZE as i32));
        let y = ST.rng.i32(16..(WORLD_SIZE as i32));
...
unsafe fn update() {
    ...
    if is_drilling && (gamepad & BUTTON_DOWN != 0) {
        ...
        for dy in 0..1 {
            for dx in 0..(PLAYER_SIZE + 2) as i32 {
                let wx = (ST.pos.x + dx - 1) as usize;
                let wy = (ST.pos.y + dy + PLAYER_SIZE as i32) as usize;
                if wx < WORLD_SIZE && wy < WORLD_SIZE {
    ...
    if is_drilling && (gamepad & BUTTON_RIGHT != 0) {
        ...
        for dy in 0..(PLAYER_SIZE + 1) as i32 {
            for dx in 0..1 {
                let wx = (ST.pos.x - 1 + dx + PLAYER_SIZE as i32) as usize;
    ...
    ST.gold_locs.retain(|gold| {
        let collected = ST.pos.x < gold.x + 2
            && ST.pos.x + PLAYER_SIZE as i32 > gold.x
            && ST.pos.y < gold.y + 2
            && ST.pos.y + PLAYER_SIZE as i32 > gold.y;
        !collected
    });
    ...
    ST.rain_locs.retain(|rain| {
        ...
        if wx < WORLD_SIZE && wy < WORLD_SIZE {
            if let Some(cell) = ST.world_get(wx, wy) {
                if cell {
                    rain_hits.push(Pos::new(wx as i32, wy as i32));
                    return false;
                }
            }
        }
        ...
        if ST.frame % 6 == 0 {
            ...
            if let Some(below) = ST.world_get(x, y + 1) {
                if !below {
                    ...
                    for dy in 0..PLAYER_SIZE {
                        for dx in 0..PLAYER_SIZE {
                            let px = ST.pos.x + dx as i32;
                            let py = ST.pos.y + dy as i32;
                            if px == x as i32 && py == (y as i32 + 1) {
    ...
    *DRAW_COLORS = 4;
    ...
    let drill_off = match dir {
        0 => Pos::new(PLAYER_SIZE as i32, 0),
        1 => Pos::new(-(PLAYER_SIZE as i32), 0),
        2 => Pos::new(PLAYER_SIZE as i32, 0),
        3 => Pos::new(0, PLAYER_SIZE as i32),
        _ => Pos::new(PLAYER_SIZE as i32, 0),
    };
    ...
    if drill_show && is_drilling {
        ...
        blit(
            drill_sprite,
            ST.pos.x + drill_off.x,
            ST.pos.y + drill_off.y,
            8,
            PLAYER_SIZE as u32,
            drill_flags,
        );
